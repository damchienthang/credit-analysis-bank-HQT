...
const SlideHeader = ({ category, title }) => (
  <div style={{
    ...
    <div>
      <p style={{ fontSize: 11, fontWeight: 800, color: C.accent, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 2 }}>
        {category}
      </p>
      <h2 style={{ fontSize: 17.5, fontWeight: 900, color: C.navy, letterSpacing: '-0.02em', margin: 0, lineHeight: 1.25 }}>
        {title}
      </h2>
    </div>
    <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
      <p style={{ fontSize: 11, fontWeight: 900, color: C.accent, letterSpacing: '0.05em', margin: 0 }}>
...
const SlideFooter = ({ pageNum }) => (
  <div style={{
    ...
    <p style={{ fontSize: 9.5, color: C.slateBlue, fontWeight: 500, margin: 0 }}>
      Ngân hàng TMCP Tín dụng Số Việt Nam (VDCB) | Khối Quản trị Rủi ro & BI Analytics Unit
    </p>
    <p style={{ fontSize: 10, color: C.accent, margin: 0, fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>
      Slide {pageNum}/15
    </p>
  </div>
);
...
const HighlightMetric = ({ label, value, change, isPositive, sub }) => (
  <div style={{
    ...
    <span style={{ fontSize: 9.5, fontWeight: 750, color: C.accent, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>
      {label}
    </span>
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 17.5, fontWeight: 800, color: C.navy }}>
        {value}
      </span>
      {change && (
        <span style={{
    ...
    <span style={{ fontSize: 9.5, color: C.slateBlue, fontWeight: 500, marginTop: 2 }}>
      {sub}
    </span>
    <div style={{
      position: 'absolute',
...
const BottomBulletInsight = ({ title, bullets }) => (
  <div style={{
    ...
    <p style={{ fontSize: 11, fontWeight: 800, color: C.navy, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 6px 0' }}>
      {title}
    </p>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 20px', fontSize: 11.5, color: C.slateBlue, lineHeight: 1.55 }}>
...
const ChapterDivider = ({ chapterNum, title, subtitle, pageNum }) => {
  // Chọn icon hoặc họa tiết bổ sung tương ứng với từng chương để tạo sự độc đáo
  const getChapterDecoration = () => {
    switch (chapterNum) {
      case 1:
        return (
          <div style={{ position: 'absolute', right: '5%', bottom: '15%', opacity: 0.08, pointerEvents: 'none' }}>
            <TrendingUp style={{ width: 280, height: 280, color: C.cyan }} />
          </div>
        );
      case 2:
        return (
          <div style={{ position: 'absolute', right: '5%', bottom: '15%', opacity: 0.08, pointerEvents: 'none' }}>
            <ShieldCheck style={{ width: 280, height: 280, color: C.cyan }} />
          </div>
        );
      case 3:
        return (
          <div style={{ position: 'absolute', right: '5%', bottom: '15%', opacity: 0.08, pointerEvents: 'none' }}>
            <Target style={{ width: 280, height: 280, color: C.cyan }} />
          </div>
        );
      default:
        return null;
  ...
  return (
    <motion.div
      ...
      style={{
        background: `linear-gradient(135deg, ${C.navy} 0%, #03112b 50%, #1652f0 100%)`,
        color: C.white,
        borderRadius: 6,
        padding: '36px 44px',
        minHeight: 520,
        ...
        overflow: 'hidden',
        boxShadow: '0 12px 36px rgba(10, 31, 68, 0.25)'
      }}
    >
      {/* Background Tech-Grid & Glowing Blobs */}
      <div style={{
        position: 'absolute',
        top: 0,
        left: 0,
        width: '100%',
        height: '100%',
        backgroundImage: `radial-gradient(circle at 80% 20%, rgba(6, 182, 212, 0.15) 0%, transparent 50%), 
                          radial-gradient(circle at 10% 80%, rgba(22, 82, 240, 0.12) 0%, transparent 50%)`,
        pointerEvents: 'none',
        zIndex: 1
      }} />

      {/* SVG Tech Grid */}
      <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.12, pointerEvents: 'none', zIndex: 1 }} xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="chapter-grid" width="30" height="30" patternUnits="userSpaceOnUse">
            <path d="M 30 0 L 0 0 0 30" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.5" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#chapter-grid)" />
        <circle cx="80%" cy="30%" r="180" fill="none" stroke="rgba(6, 182, 212, 0.2)" strokeWidth="1" strokeDasharray="5 5" />
        <circle cx="80%" cy="30%" r="280" fill="none" stroke="rgba(22, 82, 240, 0.15)" strokeWidth="1.5" />
      </svg>

      {/* Watermark Icon Decoration */}
      {getChapterDecoration()}

      {/* Top Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 2 }}>
        <span style={{ fontSize: 10.5, fontWeight: 900, color: C.cyan, letterSpacing: '0.15em' }}>VDCB INVESTOR PRESENTATION</span>
        <span style={{ fontSize: 10.5, fontWeight: 900, color: '#cbd5e0', letterSpacing: '0.08em', background: 'rgba(255,255,255,0.1)', padding: '3px 10px', borderRadius: 12 }}>PHẦN {chapterNum}</span>
      </div>

      {/* Body Section with Centered Layout (Images Removed) */}
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', margin: 'auto 0', zIndex: 2, maxWidth: 640 }}>
        <p style={{ fontSize: 13.5, fontWeight: 800, color: C.cyan, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 12, height: 2, background: C.cyan, display: 'inline-block' }} /> CHƯƠNG PHẦN {chapterNum}
        </p>
        <h1 style={{ fontSize: 26, fontWeight: 900, color: C.white, letterSpacing: '-0.02em', lineHeight: 1.25, marginBottom: 16 }}>
          {title}
        </h1>
        <div style={{ width: 50, height: 4, background: `linear-gradient(90deg, ${C.cyan} 0%, ${C.accent} 100%)`, marginBottom: 16 }} />
        <p style={{ fontSize: 13.5, color: C.lightBlueText, lineHeight: 1.6, margin: 0, fontWeight: 500 }}>
          {subtitle}
        </p>
      </div>

      {/* Bottom Footer */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.15)', paddingTop: 14, zIndex: 2 }}>
        <span style={{ fontSize: 9.5, color: '#e2e8f0', fontWeight: 500 }}>Ngân hàng TMCP Tín dụng Số Việt Nam (VDCB)</span>
        <span style={{ fontSize: 9.5, color: '#cbd5e0', fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>Slide {pageNum}/15</span>
      </div>
    </motion.div>
  );
};
  ...
  return (
    <div className="page-wrap" style={{ width: '100%', minHeight: '100vh', background: '#f7f9fc', fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      <div className="report-slides-container" style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        >
          {/* Background Decorative Tech Grid for cover */}
          <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.15, pointerEvents: 'none' }}>
            <circle cx="90%" cy="10%" r="200" fill="none" stroke={C.cyan} strokeWidth="1.5" strokeDasharray="6 6" />
            <circle cx="90%" cy="10%" r="320" fill="none" stroke={C.accent} strokeWidth="2" />
            <path d="M 0 100 L 960 100 M 0 420 L 960 420" stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
          </svg>

          {/* Logo & Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 2 }}>
            <span style={{ fontSize: 16, fontWeight: 900, color: C.white, letterSpacing: '0.05em' }}>
              VDCB <span style={{ color: C.cyan, fontWeight: 700 }}>DIGITAL CREDIT</span>
            </span>
            <span style={{ fontSize: 11, fontWeight: 800, color: '#a0aec0', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
              2018 OFFICIAL REPORT
            </span>
          </div>

          {/* Two-Column split body */}
          <div style={{ display: 'grid', gridTemplateColumns: '1.25fr 0.75fr', gap: 24, margin: 'auto 0', zIndex: 2 }}>
            {/* Left: Corporate Title & Text */}
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative' }}>
              <div style={{ width: 48, height: 4.5, background: C.cyan, marginBottom: 14 }} />
              <p style={{ fontSize: 11.5, fontWeight: 800, color: C.cyan, textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 6px 0' }}>
                BÁO CÁO CÔNG BỐ KẾT QUẢ KINH DOANH
              </p>
              <h1 style={{ fontSize: 23, fontWeight: 900, color: C.white, letterSpacing: '-0.02em', lineHeight: 1.25, marginBottom: 12 }}>
                ĐÁNH GIÁ CHẤT LƯỢNG TÀI SẢN & ĐỊNH HƯỚNG<br />
                <span style={{ color: C.cyan }}>PHÒNG VỆ RỦI RO DANH MỤC TÍN DỤNG 2018</span>
              </h1>
              <p style={{ fontSize: 13.5, color: C.lightBlueText, fontWeight: 500, lineHeight: 1.6, marginBottom: 18, margin: 0 }}>
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={val.title}>
                    <span style={{ fontSize: 11.5, fontWeight: 800, color: C.cyan, textTransform: 'uppercase', display: 'block', marginBottom: 2 }}>
                      {val.title}
                    </span>
                    <span style={{ fontSize: 10, color: '#a0aec0' }}>{val.desc}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Right: Only Large Faded Banking Icon (No Frame, with Defs Gradient) */}
            <div style={{
              height: 230,
              display: 'flex',
              alignItems: 'center',
              ...
              <motion.div
                ...
                style={{
                  ...
                  alignItems: 'center',
                  justifyContent: 'center',
                  opacity: 0.7, // Tăng độ sáng rõ rệt
          ...
          <div style={{
            ...
            {[
              ...
              <div key={idx}>
                <span style={{ fontSize: 9.5, fontWeight: 800, color: '#a0aec0', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 3 }}>
                  {item.label}
                </span>
          ...
          <div style={{ position: 'absolute', bottom: 10, right: 44, display: 'flex', alignItems: 'center', gap: 10, zIndex: 2 }}>
            <p style={{ fontSize: 9.5, color: '#a0aec0', margin: 0 }}>VDCB INVESTOR PRESENTATION © 2018</p>
          </div>
        </motion.div>
                ...
                >
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                      <span style={{ fontSize: 16, fontWeight: 900, color: C.cyan }}>{item.id}</span>
                    </div>
                    <p style={{ fontSize: 13.5, fontWeight: 800, color: C.blueText, margin: '0 0 2px 0' }}>{item.title}</p>
                    <p style={{ fontSize: 10, color: C.slateBlue, margin: 0, lineHeight: 1.35 }}>{item.desc}</p>
                  </div>
                  <div style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    ...
                    <span style={{ fontSize: 9.5, fontWeight: 700, color: C.accent }}>Slide {item.page}</span>
                    <span style={{ fontSize: 9.5, fontWeight: 800, color: C.accent, display: 'flex', alignItems: 'center' }}>Xem →</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, marginTop: 18 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontSize: 13.5, color: C.slateBlue, lineHeight: 1.65 }}>
                <p style={{ margin: 0 }}>
                  <strong>VDCB (Vietnam Digital Credit Bank)</strong> là ngân hàng thương mại cổ phần đầu tiên tại Việt Nam áp dụng mô hình vận hành số hoàn chỉnh, tối ưu hóa toàn bộ chi phí vận hành chi nhánh truyền thống để chuyển giao lợi ích lãi suất tốt nhất cho khách hàng.
                </p>

                {/* Vision / Mission */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, borderTop: `1px dashed ${C.border}`, paddingTop: 12 }}>
                  <div>
                    <span style={{ fontSize: 11.5, fontWeight: 800, color: C.accent, textTransform: 'uppercase', display: 'flex', alignItems: 'center', marginBottom: 4 }}>
                      TẦM NHÌN CHIẾN LƯỢC
                    </span>
                    <span style={{ fontSize: 12.5, color: C.slateBlue, lineHeight: 1.5 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {[
                  ...
                  { label: 'Phê duyệt', val: '100% AI Lõi', sub: 'Không phê duyệt thủ công' },
                  ...
                  <div key={idx} style={{
                    ...
                    <span style={{ fontSize: 9.5, fontWeight: 800, color: C.blueText, textTransform: 'uppercase', display: 'block', marginBottom: 2 }}>{item.label}</span>
                    <span style={{ fontSize: 16.5, fontWeight: 900, color: C.navy, display: 'block' }}>{item.val}</span>
                    <span style={{ fontSize: 10, color: C.slateBlue, marginTop: 2 }}>{item.sub}</span>
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: 13.5, color: C.slateBlue, margin: '6px 0 12px 0', lineHeight: 1.55 }}>
              VDCB cung cấp hệ sinh thái dòng sản phẩm tín dụng tiêu dùng đa dạng, đáp ứng mọi nhu cầu tài chính cá nhân với quy trình đăng ký 100% online, giải ngân tức thời sau khi mô hình AI chấm điểm và phê duyệt tự động.
            </p>
                ...
                >
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 900, color: C.accent, textTransform: 'uppercase' }}>PRODUCT {i + 1}</span>
                    </div>
                    <h3 style={{ fontSize: 13, fontWeight: 800, color: C.navy, margin: '0 0 4px 0', lineHeight: 1.3 }}>{prod.title}</h3>
                    <p style={{ fontSize: 10, color: C.slateBlue, margin: 0, lineHeight: 1.4 }}>{prod.desc}</p>
                  </div>
                  ...
                  <div style={{ borderTop: `1px dashed ${C.border}`, paddingTop: 6, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5 }}>
                      <span style={{ color: C.slateBlue }}>Hạn mức:</span>
                      <span style={{ fontWeight: 800, color: C.accent }}>{prod.limit}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10.5 }}>
                      <span style={{ color: C.slateBlue }}>Xử lý:</span>
            ...
            <div style={{ marginTop: 10, fontSize: 11, color: C.warning, display: 'flex', alignItems: 'center', justifyStyle: 'space-between', background: '#fffbeb', padding: '8px 14px', borderRadius: 4, border: '1px solid #fef3c7' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, marginTop: 10 }}>
              ...
              <div style={{ position: 'relative' }}>
                <p style={{ fontSize: 11, fontWeight: 800, color: C.blueText, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, margin: 0 }}>
                ...
                <div style={{ height: 200, width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={loanTrendData} margin={{ top: 4, right: 10, left: -25, bottom: 0 }}>
                      ...
                      <XAxis dataKey="year" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }}
        ...
        >
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 10 }}>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Legend layout="vertical" verticalAlign="middle" align="right"
                        wrapperStyle={{ fontSize: 10.5, fontWeight: 700, color: C.navy }} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <div>
                <p style={{ fontSize: 11, fontWeight: 800, color: C.blueText, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6, margin: 0 }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10.5, textAlign: 'left' }}>
                  <thead>
                    <tr style={{ background: C.navy, color: C.white }}>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}` }}>Hạng</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>Số khoản</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>Lãi suất TB</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>Tỷ lệ NPL</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={row.g} style={{ background: i % 2 === 1 ? C.bg : C.white }}>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, color: C.navy }}>
                          <span>Hạng {row.g}</span>
                          <span style={{ fontSize: 8, fontWeight: 800, padding: '1px 4px', borderRadius: 2, background: i >= 4 ? '#fee2e2' : '#e0f2fe', color: i >= 4 ? C.danger : C.accent }}>
                            {row.label}
                          </span>
                        </td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.slateBlue }}>{row.count}</td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600, color: C.navy }}>{row.rate}</td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 700, color: i >= 4 ? C.danger : C.accent }}>{row.npl}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, marginTop: 10 }}>
                ...
                <div style={{ height: 200, width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={nplTrendData} margin={{ top: 4, right: 10, left: -25, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="2 4" vertical={false} stroke="#eef0f4" />
                      <XAxis dataKey="year" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }} />
                      <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }}
                        tickFormatter={v => `${v}%`} domain={[0, 22]} />
                      <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }}
                        tickFormatter={v => `$${v}M`} />
                      <Tooltip content={<ChartTip />} />
                      <Legend wrapperStyle={{ fontSize: 10.5, fontWeight: 700 }} />
              ...
              <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10.5, textAlign: 'left' }}>
                  <thead>
                    <tr style={{ background: C.navy, color: C.white }}>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}` }}>Bang</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}` }}>Vùng</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>Khoản vay</th>
                      <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>NPL %</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stateRiskData.slice(0, 5).map((row, i) => (
                      <tr key={row.state} style={{ background: i % 2 === 1 ? C.bg : C.white }}>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, fontWeight: 700, color: C.navy }}>{row.state}</td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, color: C.slateBlue }}>{row.region}</td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.slateBlue }}>{row.loans.toLocaleString()}</td>
                        <td style={{ padding: '4px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 700, color: C.danger }}>{row.npl}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 24, marginTop: 10 }}>
                ...
                <div style={{ height: 200, width: '100%' }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart margin={{ top: 8, right: 10, left: -25, bottom: 5 }}>
                      ...
                      <XAxis type="number" dataKey="dti" name="DTI" unit="%" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }} domain={[0, 55]} />
                      <YAxis type="number" dataKey="intRate" name="Lãi suất" unit="%" axisLine={false} tickLine={false} tick={{ fill: C.slateBlue, fontSize: 10 }} domain={[0, 35]} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12.5, color: C.navy }}>
                <div style={{ padding: '8px 12px', border: `1px solid ${C.warning}60`, borderRadius: 4, background: '#fffbeb', borderLeft: `3.5px solid ${C.warning}` }}>
                  <p style={{ fontSize: 12.5, fontWeight: 800, color: C.warning, margin: '0 0 4px 0' }}>PHÁT HIỆN LỖ HỔNG HỆ THỐNG:</p>
                  <p style={{ fontSize: 11.5, color: C.slateBlue, lineHeight: 1.45, margin: 0, fontWeight: 500 }}>
                ...
                <div style={{ padding: '8px 12px', border: `1px solid ${C.accent}40`, borderRadius: 4, background: '#eff6ff', borderLeft: `3.5px solid ${C.accent}` }}>
                  <p style={{ fontSize: 12.5, fontWeight: 800, color: C.accent, margin: '0 0 4px 0' }}>GIẢI PHÁP TÍCH HỢP CORE-SYSTEM:</p>
                  <p style={{ fontSize: 11.5, color: C.slateBlue, lineHeight: 1.45, margin: 0, fontWeight: 500 }}>
          ...
          <div>
            ...
            <div style={{ marginTop: 6, position: 'relative' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10, textAlign: 'left' }}>
                <thead>
                  <tr style={{ background: C.navy, color: C.white }}>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}` }}>Chỉ số hoạt động danh mục (triệu USD / %)</th>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>2015</th>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>2016</th>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>2017</th>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>2018</th>
                    <th style={{ padding: '5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>Thay đổi 18/17</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, fontWeight: 700, background: C.bg }} colSpan="6">I. QUY MÔ HOẠT ĐỘNG</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 12 }}>Số lượng khoản vay mới giải ngân (khoản)</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>420,668</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>433,873</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>443,050</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600 }}>494,510</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>+11.6%</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 12 }}>Doanh số giải ngân mới trong năm (triệu USD)</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$6,407.1</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$6,386.9</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$6,569.3</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600 }}>$7,917.4</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>+20.5%</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, fontWeight: 700, background: C.bg }} colSpan="6">II. CHẤT LƯỢNG TÀI SẢN & RỦI RO</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 12 }}>Tỷ lệ nợ xấu toàn danh mục (NPL)</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.danger, fontWeight: 500 }}>18.59%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.danger, fontWeight: 500 }}>16.44%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.warning, fontWeight: 500 }}>9.42%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>1.90%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>-7.52%</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 12 }}>Số dư nợ xấu mới phát sinh (triệu USD)</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$1,191.2</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$1,050.2</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>$618.8</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600 }}>$150.8</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>-75.6%</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, fontWeight: 700, background: C.bg }} colSpan="6">III. VẬN HÀNH & HIỆU QUẢ TÀI CHÍNH</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 16 }}>Lãi suất giải ngân danh mục bình quan năm</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>12.60%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>12.90%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>13.10%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600 }}>13.00%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>-0.10%</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, paddingLeft: 16 }}>Tỷ lệ thu hồi nợ xấu trung bình thực tế</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>7.42%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>7.85%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right' }}>8.12%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', fontWeight: 600 }}>8.35%</td>
                    <td style={{ padding: '4.5px 8px', border: `1px solid ${C.border}`, textAlign: 'right', color: C.accent, fontWeight: 700 }}>+0.23%</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 10 }}>
              <div>
                <p style={{ fontSize: 11.5, fontWeight: 800, color: C.danger, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4, margin: 0 }}>
                  KHUYẾN NGHỊ KHẨN CẤP (Hành động trong 30 ngày):
                </p>
                <div style={{ fontSize: 10.8, color: C.slateBlue, lineHeight: 1.45, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <p style={{ margin: 0 }}>
              ...
              <div>
                <p style={{ fontSize: 11.5, fontWeight: 800, color: C.warning, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4, margin: 0 }}>
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: 13.5, color: C.slateBlue, margin: '6px 0 14px 0', lineHeight: 1.55 }}>
                ...
                >
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ fontSize: 11, fontWeight: 900, color: C.accent }}>{item.stage}</span>
                    </div>
                    <h4 style={{ fontSize: 13, fontWeight: 800, color: C.navy, margin: '4px 0' }}>
                      {item.title}
                    </h4>
                    <p style={{ fontSize: 10, color: C.slateBlue, margin: 0, lineHeight: 1.45 }}>{item.desc}</p>
                  </div>
                  <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 9, color: C.blueText, fontWeight: 600 }}>Trạng thái: Đang chuẩn bị</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Cam kết định hướng */}
            <div style={{ background: C.bg, border: `1px solid ${C.border}`, padding: '8px 14px', borderRadius: 4, fontSize: 11, color: C.slateBlue, marginTop: 8 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 18 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <p style={{ fontSize: 12, fontWeight: 800, color: C.accent, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 4px 0' }}>
                ...
                {[
                  ...
                  <div key={idx}>
                    <p style={{ fontSize: 13.5, fontWeight: 800, color: C.navy, margin: '0 0 2px 0' }}>{item.title}</p>
                    <p style={{ fontSize: 11.8, color: C.slateBlue, margin: 0, lineHeight: 1.45, fontWeight: 500 }}>{item.desc}</p>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <p style={{ fontSize: 12, fontWeight: 800, color: C.danger, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 4px 0' }}>
        ...
        >
          ...
          <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0.12, pointerEvents: 'none' }}>
            <circle cx="85%" cy="80%" r="180" fill="none" stroke={C.cyan} strokeWidth="1" strokeDasharray="4 4" />
            <circle cx="85%" cy="80%" r="280" fill="none" stroke={C.accent} strokeWidth="1.5" />
            <path d="M 100 0 L 100 520 M 860 0 L 860 520" stroke="rgba(255,255,255,0.03)" strokeWidth="0.8" />
          </svg>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 2 }}>
            <span style={{ fontSize: 11, fontWeight: 900, color: C.cyan, letterSpacing: '0.15em' }}>VDCB DIGITAL BANK</span>
            <span style={{ fontSize: 11, fontWeight: 900, color: '#a0aec0', letterSpacing: '0.08em', background: 'rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: 10 }}>FINISH</span>
          </div>

          <div style={{ textAlign: 'left', margin: 'auto 0', maxWidth: 680, zIndex: 2 }}>
            <div style={{ width: 40, height: 4.5, background: C.cyan, marginBottom: 20 }} />
            <p style={{ fontSize: 14, fontWeight: 800, color: C.cyan, textTransform: 'uppercase', letterSpacing: '0.12em', margin: '0 0 6px 0' }}>
              TÀI LIỆU KHÉP LẠI PHIÊN HỌP
            </p>
            <h1 style={{ fontSize: 38, fontWeight: 900, color: C.white, letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: 12 }}>
              TRÂN TRỌNG CẢM ƠN!
            </h1>
            <p style={{ fontSize: 15, color: C.lightBlueText, fontWeight: 500, lineHeight: 1.65, marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 12.5, color: C.cyan, fontWeight: 700, letterSpacing: '0.05em', margin: 0 }}>
              Mọi câu hỏi và ý kiến phản hồi xin liên hệ: Phòng Quan hệ Nhà đầu tư (IR Department) · ir@vdcb.com.vn
            </p>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 16, zIndex: 2 }}>
            <span style={{ fontSize: 10, color: '#a0aec0' }}>Báo cáo Công bố Kết quả Kinh doanh & Quản trị Rủi ro Q4/2018</span>
            <span style={{ fontSize: 10, color: '#a0aec0', fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>Slide 15/15</span>
          </div>
        </motion.div>
